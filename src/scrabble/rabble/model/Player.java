...
public class Player extends Sendable {
	...
	int identifier; // Integer value used to determine players, should be the
					// number of players when joined
	int age; // Order is determined by age
	int points; // Number of points a player has
	int avatar = -1; // 0-8 representing a picture, -1 = null
	boolean isTurn = false; // boolean controlling if the player is able to play
							// or not
	...
	public void showTiles() {
		...
		for (int i = 0; i < tiles.size(); i++)
			temp += tiles.get(i).getValue() + ", ";
		System.out.println(temp);
		System.out.println("_________________");
	}

	public void removeTile(Tile t, int index) {
	...
	public void replace(ArrayList<Tile> playedTiles, ArrayList<Tile> newTiles) {
		...
		for (int i = 0; i < newTiles.size(); i++)
			temp.add(newTiles.get(i));
		tiles = temp;
	}

	/*
	 * // remove elements in s[], replace with ns[] public void
	 * replace(ArrayList<Tile> playedTiles, ArrayList<Tile> newTiles){
	 * ArrayList<Tile> temp = new ArrayList<Tile>(); int counter = 0; for (int i
	 * = 0; i < tiles.size(); i++){ if(counter >= playedTiles.size()) break; //
	 * if x is outside of bounds else if(tiles.get(i) ==
	 * playedTiles.get(counter)){ // if this element is to be removed
